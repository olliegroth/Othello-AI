...
class SimpleAi:
    ...
    def MakeAiMove(self, game):
        listOfValidMoves = game.FindValidMoves(self.symbol) # The list of all valid moves for the Ai
        if listOfValidMoves:
            chosenMove = self.FindBestAiMove(listOfValidMoves) # Runs separate methods to find the best move
            coords = chosenMove.GetCoords() # Gets the coords for the chosen move
            # Carries out the move
            if chosenMove:
                game.board.PlaceDisk(coords, self.symbol)
                print("\nAi played move:",chosenMove)
                for tile in chosenMove.tilesToFlip:
                    # print("\nFlipped tiles:",tile,"\n")
                    game.FlipTiles(tile, self.symbol)
                game.board.PrintBoard()
                return True
    
    def FindBestAiMove(self, listOfValidMoves):
        cornerTiles = self.CornerTiles(listOfValidMoves) # Checks for corner tiles
        maxResult = self.Maximise(listOfValidMoves) # Runs a maximising algorithm
        if cornerTiles: # Always picks a corner tile if available
            return cornerTiles
        else: # Otherwise, maximise the tiles flipped
            return maxResult

    def CornerTiles(self, listOfValidMoves): # Finds corner tiles for the Ai
    ...
    def Maximise(self, listOfValidMoves):
        # listOfValidMoves = game.FindValidMoves(self.symbol)
        print("\nAi Valid Moves: ",listOfValidMoves)
        # Ensures that all lines are considered in the amount of tiles that can be flipped
        amountOfTiles = 0
        for move in listOfValidMoves:
            length = 0
            for row in move.tilesToFlip:
                length = length + len(row)
            print("\nMove:",move," | Flipped Tiles: ",move.tilesToFlip," | Amount of Flipped Tiles: ", length) # Print details for each move
            if length > amountOfTiles: # Choses the move which flips the most tiles
                amountOfTiles = length
                bestMove = move
        return bestMove
...
class Game(): # Contains all game logic methods
    def __init__(self):
        ...
        self.turn = "Player" # Whos turn it is
        self.state = "Running" # The state of the game

    symbols = ["●", "○"] # Varibles used across the game, hence not "self.symbols"
    ...
    def NewGame(self, generateBoard=True): # Generate board is there so that the board won't be reset when loading a game
        ...
        while self.state != "Finished":
            if self.turn == "Player":
                self.MakeMove(self.turn)
            else:
                SimpleAi(self.symbols[1]).MakeAiMove(self)
            self.SwitchTurns()
    ...
    # They return T/F if a line can be created by iterating ___ direction from a given location
    # 
    ...
    def MakeMove(self, turn):
        ...
        while moveBeingPlayed: 
            ...
            while userInput == False:
                userCoords = input("\n"f'{turn}, place a disk [Enter S to Save and Quit]: ')
            ...
            if moveIsGood:
                ...
                for tile in moveIsGood.tilesToFlip:
                    print("\nFlipped tiles:",tile,"\n")
